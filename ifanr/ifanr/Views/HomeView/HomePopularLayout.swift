...
struct HomePopularLayout: Initable {
    ...
    fileprivate func calculateDateSize() -> CGSize {
        let dateString = "\(model.category) | \(Date.getDate(model.pubDate)))"
        let toolBarAttribute = [NSFontAttributeName: UIFont.customFont_FZLTXIHJW(fontSize: 12)]
        return (dateString as NSString).boundingRect(with: CGSize(width: 200, height: kHomeCellToolBarHeight), options: .usesLineFragmentOrigin, attributes: toolBarAttribute, context: nil).size
    }
